...
class Group {
    ...
    if (restack) {
      ...
      if (typeof this.itemSet.options.order === "function") {
        ...
        if (this.doInnerStack && this.itemSet.options.stackSubgroups) {
          ...
          if (stackContextUnchanged && this._stackMembershipUnchanged()) {
            // pure horizontal pan over the same stacked items: the previous
            // vertical positions are still valid
          } else {
  ...
  _computeVerticalBand() {
    if (this.groupId !== UNGROUPED || !this.itemSet.options.showUngroupedItems) {
      return null;
    }
  ...
  show() {
    ...
    if (!this.dom.background.parentNode && this.groupId !== UNGROUPED) {
      this.itemSet.dom.background.appendChild(this.dom.background);
    }

    if (this.groupId === UNGROUPED && !this.dom.ungrouped.parentNode) {
  ...
  _updateItemsInRange(orderedItems, oldVisibleItems, range) {
    ...
    const lowerBound =
      this.groupId === UNGROUPED ? visibilityRange.start : range.start - interval;
    const upperBound =
      this.groupId === UNGROUPED ? visibilityRange.end : range.end + interval;
